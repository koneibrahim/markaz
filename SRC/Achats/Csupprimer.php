  ...
  echo'<div id="caisse2">';
	...
	//***********  <!-- Modal_sup --> **************
      ...
      echo'<div class="modal-content ">
        <div class="modal-header" style="padding:35px 50px;">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4><span class="glyphicon glyphicon-trash"></span> Suppression</h4>
        </div>';
        echo'<div class="modal-body" style="padding:40px 50px;">
          <form class="form-horizontal " role="form" action="contenuac.php" method="post">';
          ...
          echo' <input type="hidden" name="qte_liv" value="'.$qte_liv.'">';
          echo' <input type="hidden" name="montant" value="'.$montant.'">';
          
          echo'<h2>Voulez-vous retirer le produit <b class="rouge">'.$nom_pro.'</b> de l\'achat ?</h2>';
          	if($_SESSION['utilisateur']=='Identifie') {
	echo'<button type="submit" class="btn btn-danger btn-block" name="valider" value="Oui">
              <span class="glyphicon glyphicon-ok"></span> OUI</button>
          </form>
        </div>';
	echo'<div class="modal-footer">
